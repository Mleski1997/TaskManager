...
export async function addTask(title, description, dueDate, status, selectedUsers, tasks, setTasks) {
	try {
		const payload = {
		...
		const response = await axios.post(`https://localhost:7219/api/ToDo`, payload, {
			headers: {
		...
		if (response.status === 200) {
			...
			clearFormFields(title, description, dueDate, status)
		} else {
			console.log(response)
		}
	} catch (error) {
